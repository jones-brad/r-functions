...
addBar <- function(vec, col.vec, val.lab = TRUE,
	...
	for (j in 1:length(vec)) {
		...
		if (j == length(vec) & addNet) {
			...
			text( x.pos + .05, pos, round(sum(vec)*100),
				family = fam, cex = .75)
		}
	}
...
###Main function for creating the stacked bar plot
...
if (write.file == "pdf") {
	...
	for (j in 1:length(col.lab)) {
		text(col.pos[j], 0, col.lab[j], family = fam,
